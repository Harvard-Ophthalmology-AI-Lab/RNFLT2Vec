...
class PConvUnet_DelContrast(object):
...
#         imputed_out = Layer(name='consist', name='consist')(output_inpaint)
        ...
        model = Model(inputs=[inputs_img, inputs_mask, inputs_img_cc, inputs_mask_cc, id_pairs], 
                      outputs=[outputs, imputed_out, projection_out])

        return model, inputs_mask, con_loss
...
#                                            batch_size = self.batch_size)
        ...
        weights = [self.w1, self.w3, 0]
#         weights = [1., 0.001, 100000.]
        print('weight', weights)
        
        model.compile(
                optimizer = Adam(lr=lr),
                loss=[reconstruct_loss, consistency_loss, self.placeholder_loss()],
                loss_weights=weights,
                metrics={'inpaint':'mse', 'consist':'mse'}
#                 metrics=[self.PSNR]
                )
